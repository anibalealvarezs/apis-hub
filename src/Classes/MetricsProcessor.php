...
class MetricsProcessor
{
    ...
    public static function processEvents(ArrayCollection $metrics, EntityManager $manager): array
    {
        // Extract events from metrics
        $events = array_filter(array_map(function ($metric) {
            return self::extractMetricLookupKey($metric, 'event');
        }, $metrics->toArray()));
        ...
        // Batch select events from list
        $selectParams = array_values($uniqueEvents);
        $selectPlaceholders = implode(', ', array_fill(0, count($selectParams), '?'));
        ...
        try {
            $existingEvents = $manager->getConnection()
                ->executeQuery($sql, $selectParams)
                ->fetchAllAssociative();
        } catch (Exception $e) {
            throw new RuntimeException("Failed to fetch existing events: ".$e->getMessage(), 0, $e);
        }

        // Map events to their IDs and create a map for quick access
        $map = [];
        foreach ($existingEvents as $event) {
            $map[$event['name']] = $event['id'];
        }

        // Get the list of events that need to be inserted
        $eventsToInsert = array_diff($uniqueEvents, array_keys($map));
        ...
        if (! empty($eventsToInsert)) {
            ...
            $sql = Helpers::buildInsertIgnoreSql(
                ...
                $cols,
                ['name'],
                count($eventsToInsert)
            );
            $manager->getConnection()->executeStatement($sql, $insertParams);
        }

        // Re-fetch ALL unique events to ensure map is complete
        try {
            $allExistingEvents = $manager->getConnection()
                ->executeQuery(sql: "SELECT id, name FROM events WHERE name IN ($selectPlaceholders)", params: $selectParams)
                ->fetchAllAssociative();
        } catch (Exception $e) {
            throw new RuntimeException("Failed to re-fetch events: ".$e->getMessage(), 0, $e);
        }

        $finalMap = [];
        $mapReverse = [];
        foreach ($allExistingEvents as $event) {
            $finalMap[$event['name']] = $event['id'];
            $mapReverse[$event['id']] = $event['name'];
        }

        return ['map' => $finalMap, 'mapReverse' => $mapReverse];
    }

    public static function processChanneledEvents(ArrayCollection $metrics, EntityManager $manager, ?array $channeledAccountMap = null, ?array $eventMap = null): array
    {
        $channel = $metrics->first()?->channel;
        $driverClass = $channel ? DriverFactory::getRegistry()[(string)$channel]['driver'] ?? null : null;
        $channelObj = Channel::tryFromName((string)$channel);
        $channelId = $channelObj ? $channelObj->getId() : (int)$channel;

        $uniqueChanneledEvents = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'channeledEvent');
            if (! $pId) {
                continue;
            }
            ...
            $eventId = $eventMap['map'][$pId] ?? null;

            $key = $channelId . '_' . $pId . '_' . $channeledAccountId;
            $uniqueChanneledEvents[$key] = [
                'platform_id' => $pId,
                'channel' => $channelId,
        ...
        if (empty($uniqueChanneledEvents)) {
            return ['map' => [], 'mapReverse' => [], 'global' => []];
        }

        $platformIds = array_unique(array_column($uniqueChanneledEvents, 'platform_id'));
        $placeholders = implode(',', array_fill(0, count($platformIds), '?'));
        $sql = "SELECT id, platform_id, event_id FROM channeled_events WHERE platform_id IN ($placeholders) AND channel = ?";
        ...
        foreach ($results as $row) {
            ...
            $map['global'][$row['platform_id']] = (int)($row['event_id'] ?? 0);
            $mapReverse[$row['id']] = $row['platform_id'];
        }

        $toInsert = [];
        foreach ($uniqueChanneledEvents as $key => $ce) {
            if (!isset($map[$ce['platform_id']])) {
                $toInsert[] = $ce;
            }
        }

        if (!empty($toInsert)) {
            $cols = ['platform_id', 'channel', 'channeled_account_id', 'event_id', 'created_at', 'updated_at'];
            $now = date('Y-m-d H:i:s');
            $insertParams = [];
            foreach ($toInsert as $q) {
                $insertParams[] = $q['platform_id'];
                $insertParams[] = $q['channel'];
                $insertParams[] = $q['channeled_account_id'];
                $insertParams[] = $q['event_id'];
                $insertParams[] = $now;
                $insertParams[] = $now;
            }
            $sql = Helpers::buildInsertIgnoreSql(
                'channeled_events',
                $cols,
                ['platform_id', 'channel', 'channeled_account_id'],
                count($toInsert)
            );
            $manager->getConnection()->executeStatement($sql, $insertParams);

            $results = $manager->getConnection()->executeQuery(
                "SELECT id, platform_id, event_id FROM channeled_events WHERE platform_id IN ($placeholders) AND channel = ?",
                $params
            )->fetchAllAssociative();
        ...
        return ['map' => $map, 'mapReverse' => $mapReverse, 'global' => $map['global']];
    }

    public static function processLocations(ArrayCollection $metrics, EntityManager $manager): array
    {
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'location');
            if (! $pId) {
                continue;
            }
        ...
        $placeholders = implode(',', array_fill(0, count($platformIds), '?'));
        $sql = "SELECT id, platform_id FROM locations WHERE platform_id IN ($placeholders)";
    ...
    public static function processStates(ArrayCollection $metrics, EntityManager $manager): array
    {
        return ['map' => [], 'mapReverse' => []];
    }

    public static function processCities(ArrayCollection $metrics, EntityManager $manager): array
    {
        return ['map' => [], 'mapReverse' => []];
    }
    ...
    public static function processAccounts(ArrayCollection $metrics, EntityManager $manager, ?LoggerInterface $logger = null): array
    {
        ...
        $names = [];
        foreach ($metrics as $metric) {
            $locationLookupKey = self::extractMetricLookupKey($metric, 'location');
            $locationId = null;
            if ($locationLookupKey && isset($locationMap['map'][$locationLookupKey])) {
                $locationId = (int)$locationMap['map'][$locationLookupKey];
            }

            $stateLookupKey = self::extractMetricLookupKey($metric, 'state');
            $stateId = null;
            if ($stateLookupKey && isset($stateMap['map'][$stateLookupKey])) {
                $stateId = (int)$stateMap['map'][$stateLookupKey];
            }

            $cityLookupKey = self::extractMetricLookupKey($metric, 'city');
            $cityId = null;
            if ($cityLookupKey && isset($cityMap['map'][$cityLookupKey])) {
                $cityId = (int)$cityMap['map'][$cityLookupKey];
            }

            $countryLookupKey = self::extractMetricLookupKey($metric, 'country');

            $pId = self::extractMetricLookupKey($metric, 'account');
    ...
    public static function processChanneledAccounts(ArrayCollection $metrics, EntityManager $manager, ?LoggerInterface $logger = null): array
    {
        ...
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'channeledAccount');
            if (! $pId) {
                continue;
            }
    ...
    public static function processCampaigns(ArrayCollection $metrics, EntityManager $manager): array
    {
        ...
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'campaign');
            if (! $pId) {
                continue;
            }
    ...
    public static function processChanneledCampaigns(ArrayCollection $metrics, EntityManager $manager): array
    {
        ...
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'channeledCampaign');
            if (! $pId) {
                continue;
            }
    ...
    public static function processChanneledAdGroups(ArrayCollection $metrics, EntityManager $manager): array
    {
        ...
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'channeledAdGroup');
            if (! $pId) {
                continue;
            }
    ...
    public static function processChanneledAds(ArrayCollection $metrics, EntityManager $manager): array
    {
        ...
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'channeledAd');
            if (! $pId) {
                continue;
            }
    ...
    public static function processCreatives(ArrayCollection $metrics, EntityManager $manager): array
    {
        ...
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'creative');
            if (! $pId) {
                continue;
            }
    ...
    public static function processPages(ArrayCollection $metrics, EntityManager $manager): array
    {
        ...
        $canonicalIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'page');
            if ($pId) {
                if ($driverClass) {
    ...
    public static function processPosts(ArrayCollection $metrics, EntityManager $manager): array
    {
        ...
        $platformIds = [];
        foreach ($metrics as $metric) {
            $pId = self::extractMetricLookupKey($metric, 'post');
            if (! $pId) {
                continue;
            }
    ...
    public static function processMetricConfigs(
        ...
        bool             $processLocations = false,
        bool             $processStates = false,
        bool             $processCities = false,
        ...
        bool             $processChanneledEvents = false,
        ...
        // Map locations
        if ($processLocations && ! $locationMap) {
            $locationMap = self::processLocations($metrics, $manager);
        }

        // Map states
        if ($processStates && ! $stateMap) {
            $stateMap = self::processStates($metrics, $manager);
        }

        // Map cities
        if ($processCities && ! $cityMap) {
            $cityMap = self::processCities($metrics, $manager);
        }
        ...
        // Map events
        if ($processEvents && ! $eventMap) {
            $eventMap = self::processEvents($metrics, $manager);
        }

        // Map channeled events
        if ($processChanneledEvents && ! $channeledEventMap) {
            $channeledEventMap = self::processChanneledEvents($metrics, $manager, $channeledAccountMap, $eventMap);
        }
        ...
        foreach ($metrics->toArray() as $metric) {
            ...
            $locationLookupKey = self::extractMetricLookupKey($mObj, 'location');
            $locationId = null;
            if ($locationLookupKey && isset($locationMap['map'][$locationLookupKey])) {
                $locationId = (int)$locationMap['map'][$locationLookupKey];
            }

            $stateLookupKey = self::extractMetricLookupKey($mObj, 'state');
            $stateId = null;
            if ($stateLookupKey && isset($stateMap['map'][$stateLookupKey])) {
                $stateId = (int)$stateMap['map'][$stateLookupKey];
            }

            $cityLookupKey = self::extractMetricLookupKey($mObj, 'city');
            $cityId = null;
            if ($cityLookupKey && isset($cityMap['map'][$cityLookupKey])) {
                $cityId = (int)$cityMap['map'][$cityLookupKey];
            }

            $countryLookupKey = self::extractMetricLookupKey($mObj, 'country');

            $metricConfigKey = KeyGenerator::generateMetricConfigKey(
                channel: $mObj->channel ?? null,
                name: $mObj->name ?? null,
                period: $mObj->period ?? null,
                account: (self::extractMetricLookupKey($mObj, 'account', $logger)) ?: null,
                channeledAccount: (self::extractMetricLookupKey($mObj, 'channeledAccount', $logger)) ?: null,
                campaign: (self::extractMetricLookupKey($mObj, 'campaign', $logger)) ?: null,
                channeledCampaign: (self::extractMetricLookupKey($mObj, 'channeledCampaign', $logger)) ?: null,
                channeledAdGroup: (self::extractMetricLookupKey($mObj, 'channeledAdGroup', $logger)) ?: null,
                channeledAd: (self::extractMetricLookupKey($mObj, 'channeledAd', $logger)) ?: null,
                creative: (self::extractMetricLookupKey($mObj, 'creative', $logger)) ?: null,
                page: (self::extractMetricLookupKey($mObj, 'page', $logger)) ?: null,
                query: self::extractMetricLookupKey($mObj, 'query', $logger),
                post: self::extractMetricLookupKey($mObj, 'post', $logger),
                product: (self::extractMetricLookupKey($mObj, 'product', $logger)) ?: null,
                customer: (self::extractMetricLookupKey($mObj, 'customer', $logger)) ?: null,
                order: (self::extractMetricLookupKey($mObj, 'order', $logger)) ?: null,
                event: (self::extractMetricLookupKey($mObj, 'event', $logger)) ?: null,
                channeledEvent: (self::extractMetricLookupKey($mObj, 'channeledEvent', $logger)) ?: null,
            ...
            $uniqueMetricConfigs[$metricConfigKey] = [
                ...
                'event_id' => self::resolveEventId($metric, $eventMap) ?? ($channeledEventMap['global'][self::extractMetricLookupKey($metric, 'channeledEvent')] ?? null),
                'channeled_event_id' => self::resolveChanneledEventId($metric, $channeledEventMap),
                'key' => $metricConfigKey,
            ];
        }
        ...
        if (! empty($uniqueMetricConfigs)) {
            ...
            $cols = [
                ...
                'customer_id', 'order_id', 'location_id', 'state_id', 'city_id', 'country_id', 'device_id', 'dimension_set_id',
                'event_id', 'channeled_event_id', 'config_signature',
            ];
            $numCols = count($cols);
            ...
            foreach ($uniqueMetricConfigs as $key => $metricConfig) {
                ...
                $buffer[] = $metricConfig['location_id'] ?? null;
                $buffer[] = $metricConfig['state_id'] ?? null;
                $buffer[] = $metricConfig['city_id'] ?? null;
                $buffer[] = $metricConfig['country_id'] ?? null;
                $buffer[] = $metricConfig['device_id'] ?? null;
                $buffer[] = $metricConfig['dimension_set_id'] ?? null;
                $buffer[] = $metricConfig['event_id'] ?? null;
                $buffer[] = $metricConfig['channeled_event_id'] ?? null;
    ...
    private static function getMetricRelationValue(object $metric, string $property, bool $preferPlatformProp = false): mixed
    {
        $mContext = (is_callable([$metric, 'getContext'])) ? $metric->getContext() : (array)$metric;
        $platformProp = $property.'PlatformId';

        $propertyValue = $mContext[$property] ?? ($metric->$property ?? null);
        $platformValue = $mContext[$platformProp] ?? ($metric->$platformProp ?? null);

        return $preferPlatformProp ? ($platformValue ?? $propertyValue) : ($propertyValue ?? $platformValue);
    }

    private static function extractLookupKeyFromEntity(Entity $entity, string $property): ?string
    {
        return match ($property) {
            'page' => $entity instanceof Page ? ($entity->getCanonicalId() ?: (string)$entity->getPlatformId()) : null,
            'account' => $entity instanceof Account ? $entity->getName() : null,
            'channeledAccount' => $entity instanceof ChanneledAccount ? (string)$entity->getPlatformId() : null,
            'campaign' => $entity instanceof Campaign ? $entity->getCampaignId() : null,
            'channeledCampaign' => $entity instanceof ChanneledCampaign ? (string)$entity->getPlatformId() : null,
            'channeledAdGroup' => $entity instanceof ChanneledAdGroup ? (string)$entity->getPlatformId() : null,
            'channeledAd' => $entity instanceof ChanneledAd ? (string)$entity->getPlatformId() : null,
            'creative' => $entity instanceof Creative ? $entity->getCreativeId() : null,
            'post' => $entity instanceof Post ? $entity->getPostId() : null,
            'product' => $entity instanceof Product ? $entity->getProductId() : null,
            'customer' => $entity instanceof Customer ? $entity->getEmail() : null,
            'order' => $entity instanceof Order ? $entity->getOrderId() : null,
            'event' => $entity instanceof Event ? $entity->getName() : null,
            'channeledEvent' => $entity instanceof ChanneledEvent ? (string)$entity->getPlatformId() : null,
            'query' => $entity instanceof Query ? $entity->getQuery() : null,
            default => null,
        };
    }

    private static function resolveEntityIdFromMetricValue(object $metric, string $property): ?int
    {
        $rawValue = self::getMetricRelationValue($metric, $property);
        ...
        return match ($property) {
            'page' => $rawValue instanceof Page ? $rawValue->getId() : null,
            'account' => $rawValue instanceof Account ? $rawValue->getId() : null,
            'channeledAccount' => $rawValue instanceof ChanneledAccount ? $rawValue->getId() : null,
            'campaign' => $rawValue instanceof Campaign ? $rawValue->getId() : null,
            'channeledCampaign' => $rawValue instanceof ChanneledCampaign ? $rawValue->getId() : null,
            'channeledAdGroup' => $rawValue instanceof ChanneledAdGroup ? $rawValue->getId() : null,
            'channeledAd' => $rawValue instanceof ChanneledAd ? $rawValue->getId() : null,
            'creative' => $rawValue instanceof Creative ? $rawValue->getId() : null,
            'post' => $rawValue instanceof Post ? $rawValue->getId() : null,
            'product' => $rawValue instanceof Product ? $rawValue->getId() : null,
            'customer' => $rawValue instanceof Customer ? $rawValue->getId() : null,
            'order' => $rawValue instanceof Order ? $rawValue->getId() : null,
            'event' => $rawValue instanceof Event ? $rawValue->getId() : null,
            'channeledEvent' => $rawValue instanceof ChanneledEvent ? $rawValue->getId() : null,
            'query' => $rawValue instanceof Query ? $rawValue->getId() : null,
            default => $rawValue->getId(),
        };
    }

    private static function extractMetricLookupKey(object $metric, string $property, ?LoggerInterface $logger = null): ?string
    {
        $rawValue = self::getMetricRelationValue($metric, $property);
        if ($rawValue instanceof Entity) {
            return self::extractLookupKeyFromEntity($rawValue, $property);
        }

        $preferPlatformProp = [
            'page', 'account', 'channeledAccount', 'campaign', 'channeledCampaign',
            'channeledAdGroup', 'channeledAd', 'creative', 'post', 'product', 'customer', 'order', 'event', 'channeledEvent'
        ];
        ...
        if (! is_object($primary)) {
            ...
            $methods = match ($property) {
                ...
                'channeledAccount', 'channeledCampaign', 'channeledAdGroup', 'channeledAd', 'channeledEvent' => ['getPlatformId', 'getId'],
                ...
                'event' => ['getName', 'getPlatformId', 'getId'],
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'channeledAccount');
        if ($resolvedEntityId) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'page');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $pageMap) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'account');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'campaign');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'channeledCampaign');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'channeledAdGroup');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'channeledAd');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'query');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
        ...
        $pId = self::extractMetricLookupKey($metric, 'query');
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'creative');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'post');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'product');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
        ...
        $pId = self::extractMetricLookupKey($metric, 'product');
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'customer');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
        ...
        $pId = self::extractMetricLookupKey($metric, 'customer');

        return $pId ? ($map['map'][$pId] ?? null) : null;
    }

    private static function resolveChanneledEventId(object $metric, ?array $map): ?int
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'channeledEvent');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
    ...
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'order');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
        ...
        $pId = self::extractMetricLookupKey($metric, 'order');
    ...
    private static function resolveEventId(object $metric, ?array $map): ?int
    {
        $resolvedEntityId = self::resolveEntityIdFromMetricValue($metric, 'event');
        if ($resolvedEntityId) {
            return $resolvedEntityId;
        }
        if (! $map) {
        ...
        $pId = self::extractMetricLookupKey($metric, 'event');
