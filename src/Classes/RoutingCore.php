    ...
    class RoutingCore implements HttpKernelInterface
    {
        ...
        public function handle(Request $request, int $type = self::MAIN_REQUEST, bool $catch = true): Response
        {
            ...
            try {
                ...
                $attributes['isDemo'] = Helpers::isDemo();
                $attributes['channelsConfig'] = Helpers::getChannelsConfig();
        ...
        {
            // 1. IP Whitelisting
            $authorizedIps = Helpers::getAuthorizedIps();
            if (!empty($authorizedIps)) {
                $clientIp = $request->getClientIp();
                if (!IpUtils::checkIp($clientIp, $authorizedIps)) {
                    return false;
                }
            }
        ...
        private function shouldRateLimit(Request $request): bool
        {
            ...
            if (Helpers::isDemo()) return false;
            ...
            $driverConfigs = DriverFactory::getRegistry();
            foreach ($driverConfigs as $config) {
                $driverClass = $config['driver'] ?? null;
                if ($driverClass && method_exists($driverClass, 'getRateLimitWhitelist')) {
                    $whiteList = array_merge($whiteList, $driverClass::getRateLimitWhitelist());
                }
            }
