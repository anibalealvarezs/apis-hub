<?php
    ...
    class InstanceGeneratorService
    {
        ...
        {
            $instances = [];
            Helpers::getProjectConfig(); // Asegurar que el entorno está cargado
            $today = new DateTimeImmutable('today');
            $yesterday = $today->modify('-1 day');

            if (getenv('APP_ENV') === 'demo' || Helpers::isDemo()) {
                return [[
            ...
            foreach ($availableChannels as $channelName) {
                ...
                try {
                    $chanConfig = DriverInitializer::validateConfig($channelName);
                } catch (Exception $e) {
                ...
                $driverClass = DriverFactory::getChannelConfig($channelName)['driver'] ?? null;
                $rules = [];
        ...
        private function hasActiveEntities(string $channelName): bool
        {
            ...
            try {
                $config = DriverInitializer::validateConfig($chanKey);
            } catch (Exception $e) {
                return false;
            }
