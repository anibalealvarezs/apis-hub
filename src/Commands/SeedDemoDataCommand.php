    ...
    class SeedDemoDataCommand extends Command implements SeederInterface
    {
        ...
        public function processMetricsMassive(Collection $metrics): void
        {
            ...
            try {
                ...
                $this->entityManager->flush();
                $this->conn->commit();
            } catch (Throwable|\Doctrine\DBAL\Exception $e) {
        ...
        protected function execute(InputInterface $input, OutputInterface $output): int
        {
            ...
            foreach ($channels as $chanName) {
                try {
                    $driver = DriverFactory::get($chanName);
                    if ($driver instanceof SyncDriverInterface) {
        ...
        private function bulkInsert(string $table, array $rows, array $updateCols = []): void
        {
            ...
            foreach (array_chunk($rows, $chunkSize) as $chunk) {
                $sql = Helpers::buildUpsertSql($table, $columns, $updateCols, $uniqueCols, count($chunk));
                $params = [];
