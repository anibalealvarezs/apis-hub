        ...
        foreach ($instances as $instance) {
            ...
            if ($chanConfig) {
                ...
                if (! empty($instance['start_date'])) {
                    $params['startDate'] = $instance['start_date'];
                }
                if (! empty($instance['end_date'])) {
                    $params['endDate'] = $instance['end_date'];
                }
                if (! empty($instance['requires'])) {
                    $params['requires'] = $instance['requires'];
                }
                    ...
                    try {
                        ...
                        if (empty($accounts)) {
                            if (str_contains($channel, 'facebook')) {
                                echo "DEBUG: No accounts in YAML for $channel, checking DB...\n";
                            }
                            $caRepo = $this->entityManager->getRepository(ChanneledAccount::class);
                            $dbAccounts = $caRepo->findBy(['channel' => $channel]);
                            if (str_contains($channel, 'facebook')) {
                                echo "DEBUG: Found ".count($dbAccounts)." accounts in DB\n";
                            }
                        ...
                        if (str_contains($channel, 'facebook')) {
                            echo "DEBUG: Error in granular logic: ".$e->getMessage()."\n";
                        }
                    }
                }
