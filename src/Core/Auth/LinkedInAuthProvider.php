...
class LinkedInAuthProvider implements AuthProviderInterface
{
    ...
    public function __construct(?string $tokenPath = null)
    {
        ...
        $this->tokenPath = $tokenPath ?? $_ENV['LINKEDIN_TOKEN_PATH'] ?? $projectDir . '/storage/tokens/linkedin_tokens.json';
    ...
    private function loadCredentials(): void
    {
        if (file_exists($this->tokenPath)) {
            ...
            $this->credentials = $tokens['linkedin_auth'] ?? [];
        }

        if (empty($this->credentials)) {
            $config = Helpers::getChannelsConfig()['linkedin'] ?? [];
            $this->credentials = [
                'client_id' => $config['linkedin_client_id'] ?? $_ENV['LINKEDIN_CLIENT_ID'] ?? '',
                'client_secret' => $config['linkedin_client_secret'] ?? $_ENV['LINKEDIN_CLIENT_SECRET'] ?? '',
                'access_token' => $config['linkedin_access_token'] ?? $_ENV['LINKEDIN_ACCESS_TOKEN'] ?? '',
                'refresh_token' => $config['linkedin_refresh_token'] ?? $_ENV['LINKEDIN_REFRESH_TOKEN'] ?? '',
            ];
        }
    }
    ...
    private function saveCredentials(): void
    {
        ...
        $tokens['linkedin_auth'] = $this->credentials;
