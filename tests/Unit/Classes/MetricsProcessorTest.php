<?php
    ...
    class MetricsProcessorTest extends BaseUnitTestCase
    {
        ...
        {
            $mockAcc = $this->createMock(Account::class);
            $mockAcc->method('getId')->willReturn(123);
            $mockAcc->method('getName')->willReturn('account_name');

            $metrics = new ArrayCollection([
                (object)['account' => $mockAcc, 'channel' => 'facebook']
            ]);

            $resultMock = $this->createMock(Result::class);
            $resultMock->method('fetchAllAssociative')->willReturn([
                ['id' => 123, 'name' => 'account_name']
            ]);
            ...
            $this->assertArrayHasKey('account_name', $res['map']);
            $this->assertEquals(123, $res['map']['account_name']);
        }

        public function testProcessChanneledAccountsReturnsMap(): void
        {
            $metrics = new ArrayCollection([
                (object)['channeledAccountPlatformId' => 'p123', 'channel' => 'facebook']
            ]);

            $resultMock = $this->createMock(Result::class);
            $resultMock->method('fetchAllAssociative')->willReturn([
                ['id' => 456, 'platform_id' => 'p123', 'account_id' => 123]
            ]);
        ...
        {
            $mockCamp = $this->createMock(Campaign::class);
            $mockCamp->method('getCampaignId')->willReturn('c789');

            $metrics = new ArrayCollection([
                (object)['campaign' => $mockCamp, 'channel' => 'facebook']
            ]);
