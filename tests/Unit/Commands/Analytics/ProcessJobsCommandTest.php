<?php
    ...
    class ProcessJobsCommandTest extends TestCase
    {
        ...
        public function testExecuteProcessesJobWhenDependencyMet(): void
        {
            ...
            $mockChannelRepository = $this->createMock(\Doctrine\ORM\EntityRepository::class);
            $mockChannelRepository->method('findOneBy')->willReturn($mockChannelEntity);
            $this->entityManager->method('getRepository')
                ->with(ChannelEntity::class)
                ->willReturn($mockChannelRepository);

            // Mock the SyncDriverInterface and its fetchData method
            $mockDriver = $this->createMock(SyncDriverInterface::class);
            $mockDriver->method('fetchData')->willReturn(new Response('{}', 200));

            // Use reflection to inject the mock driver into DriverFactory's static $drivers array
            $driverFactoryReflection = new \ReflectionClass(DriverFactory::class);
            $driversProperty = $driverFactoryReflection->getProperty('drivers');
            $driversProperty->setAccessible(true);
            $driversProperty->setValue(null, ['facebook' => $mockDriver]); // Inject for 'facebook' channel
